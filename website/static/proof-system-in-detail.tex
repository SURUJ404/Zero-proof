...
\author{Jeremy Bruestle, Paul Gafni, and the TSC Team}
\title{TSC zkVM: Scalable, Transparent Arguments of RISC-V Integrity}
...
\maketitle
\begin{abstract}
TSC offers an open source virtual machine coupled with a zero-knowledge proof system.
...
The TSC proof system implements a zk-STARK instantiated using the FRI protocol, DEEP-ALI, and an HMAC-SHA-256 based PRF. 
In this paper, we formally articulate the TSC Proof System with as few technical barriers to understanding as possible, including a detailed construction of a TSC cryptographic seal.
\end{abstract}

\tableofcontents

\section*{Acknowledgements}
Many thanks to Bolton Bailey, Lennart Beringer, Tim Carstens, Victor Graf, Tim Zerrell, and the rest of the TSC team for their support and feedback. 
...
In order to make verifiable software development possible in languages like Rust and C++, \textbf{TSC has built a mechanism for demonstrating the integrity of any RISC-V computation.} 

\subsection{Verifiable RISC-V}
When a \href{https://riscv.org/} {RISC-V} binary executes in the TSC zkVM, the output is paired with a computational receipt.
...
To ensure that the TSC verifier (which checks the receipts) is functioning without security bugs, we're working on a formal verification of the verifier implementation. 
This formal verification work is available at \url{https://www.github.com/risc0/risc0-lean4}.\\
\\
\subsection{The TSC Argument System}
TSC's argument system is based on \cite{stark}; 
the seal on a TSC receipt is a zk-STARK.
...
In this paper, we specify the TSC protocol and analyze its knowledge soundness. 
...
\section{Background}
\label{background}
The argument system behind TSC's receipts is built in terms of an \textit{execution trace} and a number of \textit{constraints} that enforce checks of computational integrity.
We start by introducing those terms as they're used in the context of the TSC protocol.
...
\\
The columns of the TSC execution trace are categorized as follows:
\begin{description}
...
\label{arguments}
The seal on the TSC receipt is a STARK -- a scalable, transparent argument of knowledge \cite{stark}. 
...
\section{The TSC IOP Protocol}
\label{iop}
In this section, we present the interactive version of the TSC protocol and analyze the knowledge soundness of the protocol in the context of the IOP model \cite{iop}. 
...
\\
The seal on a TSC receipt is the transcript of the interactive protocol, with a random oracle as the verifier.
...
\label{preliminaries}
\subsection{Cyclic Domains (Indexing in TSC)}
\label{sequential time}
\label{cyclic domains}
In this section, we define the indexing for an execution trace. TSC's architecture is finite-field based, and the indexing is based on powers of $\w\in\GF{p}$. \\
\\
...
\label{reed solomon}
In this section, we define Reed Solomon codes and outline their place in the TSC proof system.
...
\\
\\
For zero-knowledge purposes, TSC adds random padding to the trace columns before implementing Reed-Solomon encoding and evaluates the NTT on a domain that does not intersect the domain of the message. 
...
\footnote{Note that while most discussions of NTTs and iNTTs occur over a multiplicative subgroup of a finite field, TSC's NTT and iNTT implementation works over a coset of a multiplicative subgroup as well. 
...
\label{FRI}
TSC uses the batched FRI protocol as described in Section 8.2 of the Proximity Gaps for Reed-Solomon Codes paper. \\
\\
...
\\
TSC uses a \textit{split factor} of 16 throughout FRI, but we write the split factor as $l$ for generality.%
...
One important performance enhancement used in TSC's protocol involves the use of a subfield $\GF{p}$ of a larger field $\GF{q}=\GF{p^n}$ for some of the protocol.
