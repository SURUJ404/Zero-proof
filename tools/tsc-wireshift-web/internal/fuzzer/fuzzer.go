...
func (f *Fuzzer) StartFuzzer(data map[string]interface{}) {
	...
	wailsruntime.EventsEmit(f.ctx, "backend:FuzzerProgress", map[string]interface{}{
		"tabId":    int(tabId),
		"progress": 0,
	})
	...
	wailsruntime.EventsEmit(f.ctx, "backend:FuzzerFinished", map[string]interface{}{
		"tabId": runningTabId,
	})
...
func (f *Fuzzer) sendFuzzerResult(tabId, index int, allPayloadValues [][]string, resp *http.Response, err error) {
	...
	wailsruntime.EventsEmit(f.ctx, "backend:FuzzerResult", map[string]interface{}{
		"id":     tabId,
		"result": result,
	})
}
...
func (f *Fuzzer) AddFuzzerTab(tabData map[string]interface{}) {
	...
	if !ok {
		body = `{"tool": "tsc-wireshift", "test": "This is a test request", "timestamp": "[__Inject-Here__[1]]"}`
	}
	...
	wailsruntime.EventsEmit(f.ctx, "backend:FuzzerTabs", f.GetFuzzerTabs())
	wailsruntime.EventsEmit(f.ctx, "backend:newFuzzerTab", map[string]interface{}{
		"tabId": tabID,
	})
}
...
func (f *Fuzzer) UpdateFuzzerTab(tabData map[string]interface{}) {
	...
	wailsruntime.EventsEmit(f.ctx, "backend:FuzzerTabUpdated", map[string]interface{}{
		"id": int(id),
	})
}
...
func (f *Fuzzer) UpdateFuzzerTabName(tabId float64, newName string) {
	...
	wailsruntime.EventsEmit(f.ctx, "backend:FuzzerTabNameUpdated", map[string]interface{}{
		"tabId":   int(tabId),
		"newName": newName,
	})
}
...
func (f *Fuzzer) RemoveFuzzerTab(tabId int) {
	...
	if err != nil {
		wailsruntime.EventsEmit(f.ctx, "backend:FuzzerTabRemoved", map[string]interface{}{
	...
	wailsruntime.EventsEmit(f.ctx, "backend:FuzzerTabs", tabs)
	wailsruntime.EventsEmit(f.ctx, "backend:FuzzerTabRemoved", map[string]interface{}{
		"success": true,
		"tabId":   tabId,
	})
