...
func (c *Client) Poll() (bool, error) {
	...
	if data, ok := responseData["data"].([]interface{}); ok {
		for _, d := range data {
			...
			wailsruntime.EventsEmit(c.ctx, "backend:newInteraction", interaction)
		}
	} else {
...
func (c *Client) StartListening() {
	...
	go func() {
		...
		if err != nil {
			...
			wailsruntime.EventsEmit(c.ctx, "backend:registrationStatus", false)
			wailsruntime.EventsEmit(c.ctx, "backend:registrationError", err.Error())
		...
		if success {
			...
			wailsruntime.EventsEmit(c.ctx, "backend:registrationStatus", true)

			domain := c.GetInteractDomain()
			log.Printf("INFO: Generated new Interactsh domain: %s", domain)
			wailsruntime.EventsEmit(c.ctx, "backend:domain", map[string]string{"domain": domain})
			for {
				c.listeningMtx.Lock()
				if !c.isListening {
					c.listeningMtx.Unlock()
				...
				if err != nil {
					...
					wailsruntime.EventsEmit(c.ctx, "backend:registrationError", "Connection lost: "+err.Error())
					break
				}
				if !success {
			...
			wailsruntime.EventsEmit(c.ctx, "backend:registrationStatus", false)
			wailsruntime.EventsEmit(c.ctx, "backend:registrationError", "Failed to register with Interactsh server")
