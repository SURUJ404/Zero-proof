...
*/
//=======================================================
bool __TSCSOCKET__::__createserver(TSCSERVER * const & server_class, PCSTR port, int protocol, FPC_CALLBACK fpConnected, FPS_CALLBACK fpData) {
	WSADATA				wsaData;
	...
	if (i_result != 0) { 
#ifdef TSCLOGGER_PLUGIN
		ERROR("%s getaddrinfo error %d", TSCSOCKET_PREFIX, i_result); 
#else
		printf("%s getaddrinfo error %d", TSCSOCKET_PREFIX, i_result);
#endif
		return false; 
	}

	Socket_Listen		= socket(res->ai_family, res->ai_socktype, res->ai_protocol);
	if (Socket_Listen == INVALID_SOCKET) { __TSCSOCKET__::cleanWSA(res); return false;}
	...
	if (i_result == SOCKET_ERROR) { __TSCSOCKET__::cleanWSA(res, Socket_Listen); return false; }
	...
	while (1)  {
		...
		__TSCSOCKET__::s_server_res * s_params = new __TSCSOCKET__::s_server_res;
		s_params->addrRes = res;
		fpData != NULL ? s_params->fpResData = fpData : s_params->fpResData = NULL;
		s_params->SocketResponse = Socket_Client;

		CreateThread(0, 0, __TSCSOCKET__::__t__createServerRes, s_params, 0, 0);
...
void TSCSERVER::send_data(SOCKET clientSocket, const char * sendChar) {
...
void TSCSERVER::close() {
	if (closesocket(Socket_Listen) == SOCKET_ERROR)
#ifdef TSCLOGGER_PLUGIN
		ERROR("%s Couldn't close server (%d)", TSCSOCKET_PREFIX, WSAGetLastError());
#else
		printf("%s Couldn't close server (%d)", TSCSOCKET_PREFIX, WSAGetLastError());
#endif
}

bool TSCSERVER::start_server(PCSTR port, int protocol, FPC_CALLBACK fpConnected, FPS_CALLBACK fpData, bool thread) {
	if (!thread) return __TSCSOCKET__::__createserver(this, port, protocol, fpConnected, fpData);
	...
	CreateThread(0, 0, __TSCSOCKET__::__t__startserver, server_data, 0, 0);
	return true;
}

DWORD WINAPI __TSCSOCKET__::__t__startserver(LPVOID lpParams) {
	s_server_params * s_data = (struct s_server_params *)lpParams;
	__TSCSOCKET__::__createserver(s_data->s_class, s_data->port, s_data->protocol, s_data->fpConnected, s_data->fpData);
	return 0;
}


void __TSCSOCKET__::cleanWSA(PADDRINFOA pAddr, SOCKET s) {
#ifdef TSCLOGGER_PLUGIN
	ERROR("%s %d\r\n", TSCSOCKET_PREFIX, WSAGetLastError());
#else
	printf("%s %d\r\n", TSCSOCKET_PREFIX, WSAGetLastError());
#endif
