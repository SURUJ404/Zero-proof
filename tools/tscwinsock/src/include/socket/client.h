...
void TSCLIENT::close() {
	if (shutdown(Socket_Client, SD_BOTH) == SOCKET_ERROR)  {
		...
		ERROR("%s failed with error: %d\n", TSCSOCKET_CLIENT_PREFIX, WSAGetLastError());
#else
...
void TSCLIENT::listen(FP_RES fpRes, bool thread) {
	this->fp_params = fpRes;
	thread ? CreateThread(0, 0, __TSCSOCKET__::__t__listenThread, this, 0, 0) : this->__listenfunc(fpRes);
}

void TSCLIENT::__listenfunc(FP_RES fpRes) {
	int			i_result;
...
void TSCLIENT::set_toggle_callback(FPCLIENT_T pointerFunc) {
	Toggle_Callback = pointerFunc;
}

void TSCLIENT::toggle_send_callback(bool toggle) {
	b_toggle = toggle;
}

bool TSCLIENT::send_data( const char * sendChar) {
	bool		b_result;
...
bool TSCLIENT::init(PCSTR server, PCSTR port, int protocol, FPCLIENT_CB fpCB) {
	WSADATA				wsaData;
	...
	if (i_result != 0) { 
#ifdef TSCLOGGER_PLUGIN
		ERROR("%s WSA Error %d\r\n", TSCSOCKET_CLIENT_PREFIX, i_result); 
#else
	...
	for (ptr = res; ptr != NULL; ptr = ptr->ai_next) {
		...
		if (Socket_Client == INVALID_SOCKET) { __TSCSOCKET__::cleanWSA(res); return false; }

		i_result = connect(Socket_Client, ptr->ai_addr, (int)ptr->ai_addrlen);
		if (i_result == SOCKET_ERROR) {
			__TSCSOCKET__::cleanWSA(res, Socket_Client);
