...
This example is meant to serve a guide for developers interested in using the Zero Proof for zkML applications using the popular XGBoost machine learning model. The model can be trained in Python and exported as a JSON file to into the Zero Proof zkVM.  Inference is computed within the guest code, allowing developers to submit a cryptographic receipt attesting to the correct execution of the trained model without revealing either the inputs or the model parameters.
