...
/// \file
/// Defines FpExt, a finite field F_p^4, based on Fp via the irreducable
/// polynomial x^4 - 11.
...
/// Intstances of FpExt are element of a finite field F_p^4.  They are
...
/// various operations depends on the size of the field.  It has the field Fp as
/// a subfield, which means operations by the two are compatable, which is
/// important.  The irreducible polynomial was choosen to be the simpilest
/// possible one, x^4 - B, where 11 is the smallest B which makes the polynomial
/// irreducable.
struct FpExt {
  ...
  // Fp * FpExt is done as a free function due to C++'s operator overloading
  // rules.
  ...
  // properly, but it'a small enough to just hand write.
  constexpr FpExt operator*(FpExt rhs) const {
...
#undef b
  ...
  constexpr Fp elem(size_t i) const { return elems[i]; }
...
#define a in.elems
  // Compute the multiplicative inverse by basicly looking at FpExt as a
  // composite field and using the same basic methods used to invert complex
  // numbers.  We imagine that initially we have a numerator of 1, and an
  // denominator of a. i.e out = 1 / a; We set a' to be a with the first and
  // third components negated.  We then multiply the numerator and the
  // denominator by a', producing out = a' / (a * a'). By construction (a * a')
  // has 0's in it's first and third elements.  We call this number, 'b' and
  // compute it as follows.
  Fp b0 = a[0] * a[0] + BETA * (a[1] * (a[3] + a[3]) - a[2] * a[2]);
  Fp b2 = a[0] * (a[2] + a[2]) - a[1] * a[1] + BETA * (a[3] * a[3]);
  // Now, we make b' by inverting b2.  When we muliply both sizes by b', we get
  // out = (a' * b') / (b * b').  But by construcion b * b' is in fact an
  // element of Fp, call it c.
  Fp c = b0 * b0 + BETA * b2 * b2;
  // But we can now invert C direcly, and multiply by a'*b', out = a'*b'*inv(c)
  Fp ic = inv(c);
  // Note: if c == 0 (really should only happen if in == 0), our 'safe' version
  // of inverse results in ic == 0, and thus out = 0, so we have the same 'safe'
  // behavior for FpExt.  Oh, and since we want to multiply everything by ic,
  // it's slightly faster to premultiply the two parts of b by ic (2 multiplies
  // instead of 4)
  b0 *= ic;
  b2 *= ic;
...
constexpr inline Fp elem(FpExt in, size_t i) {
  return in.elem(i);
}

inline std::ostream& operator<<(std::ostream& os, const FpExt& x) {
  os << "(" << x.elems[0] << ", " << x.elems[1] << ", " << x.elems[2] << ", " << x.elems[3] << ")";
  return os;
}
