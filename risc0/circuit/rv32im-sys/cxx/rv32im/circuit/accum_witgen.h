...
template <size_t po2> struct AccumWitgenContext {
  ...
  struct RegImpl {
    FDEV void set(CTX, Fp val) DEV { data[0] = val; }
  ...
  template <typename T> FDEV inline void push(const MTHR T& argument) {
    addArgument(isValid, argument);
  }
  template <typename T> FDEV inline void pull(const MTHR T& argument) {
    addArgument(-isValid, argument);
  }
  template <typename T> FDEV inline void addArgument(Fp count, const MTHR T& argument) {
...
#undef BLOCK_TYPE
  ...
  ctx.done();
  // Do Accum top
  MDEV AccumTop<C>* accumTop =
      reinterpret_cast<MDEV AccumTop<C>*>(accum + MAX_ACCUM_PER_ROW * numRows + row);
...
FDEV void computeRowPhase2(MDEV Fp* accum,
  ...
  constexpr size_t numRows = (1 << po2);
  MDEV Fp* data = const_cast<MDEV Fp*>(dataConst);
  C ctx; // Not actually used in phase2, but needs to be passed to keep API
         // happy
  ...
  MDEV AccumTop<C>* accumTopPrev =
      reinterpret_cast<MDEV AccumTop<C>*>(accum + MAX_ACCUM_PER_ROW * numRows + prevRow);
  // If prev is not simple, let earlier node handle
  if (!accumTopPrev->isSimple()) {
    return;
  }
  ...
  while (1) {
    ...
    accumTop = reinterpret_cast<MDEV AccumTop<C>*>(accum + MAX_ACCUM_PER_ROW * numRows + row);
    if (accumTop->isSimple()) {
      break;
    }
