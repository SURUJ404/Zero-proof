...
template <typename IntT> struct PortableIntDistribution {
  IntT min;
  IntT max;

  PortableIntDistribution(IntT min, IntT max) : min(min), max(max) {}

  template <typename G> IntT operator()(G& g) const { return static_cast<IntT>(g() % (max + 1)); }
};
...
TEST(merkle, regression) {
  // Test an exact merkle tree transcript which is also tested in the Rust
  // codebase.
