...
void Prover::prove(WriteIop& iop) {
  ...
  //   }
  // }
  LOG(1, "Eval + commit check group");
  check.commit(iop);
  LOG(1, "Check root = " << check.merkle.getRoot());
  ...
  hal->zero(combos);
  for (auto& poly : polys) {
    LOG(1, "Mixing in polynomial");
    poly.mix(combos, curMix, comboMix);
  }
  LOG(1, "Mixing in check");
  check.mix(combos, curMix, comboMix);
  LOG(1, "Prepare combos (i.e. subtract coeffs)");
  ...
  LOG(1, "Prepare final polynomial");
