...
public:
  ...
  copy(IBufferPtr dst, size_t dstOffset, IBufferPtr src, size_t srcOffset, size_t count) override {
    memcpy(reinterpret_cast<uint8_t*>(static_cast<CpuBuffer*>(dst.get())->buffer) + dstOffset,
  ...
  void zero(IBufferPtr buf, size_t offset, size_t size) override {
    memset(
        reinterpret_cast<uint8_t*>(static_cast<CpuBuffer*>(buf.get())->buffer) + offset, 0, size);
  }
  ...
  void computeDataWitness(HalMatrix<Fp> data,
    ...
    computeDataWitnessCpu(
        pData.data(), pGlobals.data(), pRows.data(), pAux.data(), pTables.data(), rows.size());
  }
...
private:
  IBufferPtr allocate(size_t size) override { return std::make_shared<CpuBuffer>(size); }
