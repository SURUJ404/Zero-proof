...
template <typename T> class HalArray {
  friend class IHal;
...
template <typename T> class HalMatrix {
  friend class IHal;
  friend class DualHal;
...
public:
  ...
  // Allocations
  template <typename T> HalArray<T> allocateArray(size_t size) {
    IBufferPtr inner = allocate(size * sizeof(T));
    return HalArray<T>(inner);
  }

  template <typename T> HalArray<T> allocateArrayCopy(const std::vector<T>& in);

  template <typename T> HalMatrix<T> allocateMatrix(size_t rows, size_t cols) {
    IBufferPtr inner = allocate(rows * cols * sizeof(T));
    return HalMatrix<T>(inner, rows, cols);
  }

  // Copies
  template <typename T> void copy(HalArray<T> out, HalMatrix<T> in) {
    if (out.size() != in.size()) {
      throw std::runtime_error("Invalid copy");
    }
    copy(out.inner, out._offset, in.inner, in._offset, in._size);
  }
  template <typename T> void copy(HalMatrix<T> out, HalMatrix<T> in) {
  ...
  template <typename T> void zero(HalArray<T> out) { zero(out.inner, out._offset, out._size); }
  template <typename T> void zero(HalMatrix<T> out) {
  ...
  virtual void query(
      HalArray<Fp> out, HalMatrix<Fp> data, HalArray<Digest> tree, size_t topSize, size_t idx) = 0;
  ...
  virtual void batchPolyEval(HalArray<FpExt> out,
                             HalMatrix<Fp> coeffs,
                             HalArray<uint32_t> cols,
                             HalArray<FpExt> xs) = 0;

  // Mixing related functions
  virtual void combosMix(HalMatrix<FpExt> combos,
                         HalMatrix<Fp> coeffs,
                         HalArray<uint32_t> whichCombo,
                         FpExt cur,
                         FpExt mix) = 0;
  virtual void
  combosPrep(HalMatrix<FpExt> combos, HalArray<FpExt> eval, HalArray<EvalInfo> info, FpExt mix) = 0;
  ...
  virtual void computeDataWitness(HalMatrix<Fp> data,
                                  HalArray<Fp> globals,
                                  HalArray<RowInfo> rows,
                                  HalArray<uint32_t> aux,
                                  HalArray<uint32_t> tables) = 0;
  // Compute the accum witness from the data
  virtual void computeAccumWitness(HalMatrix<Fp> accum,
                                   HalMatrix<Fp> data,
                                   HalArray<Fp> globals,
                                   HalArray<FpExt> accMix) = 0;
  // Compute the evaluation of the constraint polynomial
  virtual void evalCheck(HalMatrix<Fp> check,
                         HalMatrix<Fp> data,
                         HalMatrix<Fp> accum,
                         HalArray<Fp> globals,
...
protected:
  ...
  virtual void
  copy(IBufferPtr dst, size_t dstOffset, IBufferPtr src, size_t srcOffset, size_t count) = 0;
  // Zero a buffer
  ...
  template <typename T> IBufferPtr extract(HalArray<T> array) { return array.inner; }
  template <typename T> size_t getOffset(HalArray<T> array) { return array._offset; }
  template <typename T> IBufferPtr extract(HalMatrix<T> matrix) { return matrix.inner; }
};
...
public:
  PinnedArrayRO(IHalPtr hal, HalArray<T> buf)
      ...
      , _offset(buf._offset)
      , _size(buf._size) {}
  ~PinnedArrayRO() { hal->unpin(buf, pbuf, _offset, _size, false); }
...
public:
  PinnedArrayBase(IHalPtr hal, HalArray<T> buf, bool sync)
      ...
      , _offset(buf._offset)
      , _size(buf._size) {}
  ~PinnedArrayBase() { hal->unpin(buf, pbuf, _offset, _size, true); }
...
template <typename T> class PinnedArrayWO : public PinnedArrayBase<T> {
public:
  PinnedArrayWO(IHalPtr hal, HalArray<T> buf) : PinnedArrayBase<T>(hal, buf, false) {}
};

template <typename T> class PinnedArrayRW : public PinnedArrayBase<T> {
public:
...
public:
  PinnedMatrixRO(IHalPtr hal, HalMatrix<T> mat)
      : hal(hal)
      , buf(mat.inner)
      , pbuf(reinterpret_cast<T*>(hal->pin(buf, 0, buf->size(), true)))
      , _rows(mat.rows())
      , _cols(mat.cols()) {}
  ~PinnedMatrixRO() { hal->unpin(buf, pbuf, 0, buf->size(), false); }
  const T& operator[](size_t i) const { return pbuf[i]; }
...
public:
  PinnedMatrixBase(IHalPtr hal, HalMatrix<T> mat, bool sync)
      : hal(hal)
      , buf(mat.inner)
      , pbuf(reinterpret_cast<T*>(hal->pin(buf, 0, buf->size(), sync)))
      , _rows(mat.rows())
      , _cols(mat.cols()) {}
  ~PinnedMatrixBase() { hal->unpin(buf, pbuf, 0, buf->size(), true); }
...
template <typename T> class PinnedMatrixWO : public PinnedMatrixBase<T> {
public:
  PinnedMatrixWO(IHalPtr hal, HalMatrix<T> buf) : PinnedMatrixBase<T>(hal, buf, false) {}
};

template <typename T> class PinnedMatrixRW : public PinnedMatrixBase<T> {
public:
  PinnedMatrixRW(IHalPtr hal, HalMatrix<T> buf) : PinnedMatrixBase<T>(hal, buf, true) {}
};

template <typename T> HalArray<T> IHal::allocateArrayCopy(const std::vector<T>& in) {
