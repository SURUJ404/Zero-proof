...
fn main() {
    ...
    let (lhs_0, lhs_1, rhs_0, rhs_1, monicirr_0, monicirr_1, prime): (
        BigUint,
        BigUint,
        BigUint,
        BigUint,
    ...
    let monicirr = [monicirr_0.to_u32_array(), monicirr_1.to_u32_array()];
    let prime = prime.to_u32_array();

    let mut result =
        [[0u32; risc0_bigint2::field::FIELD_256_WIDTH_WORDS]; risc0_bigint2::field::EXT_DEGREE_2];
    risc0_bigint2::field::extfield_deg2_mul_256(&lhs, &rhs, &monicirr, &prime, &mut result);
