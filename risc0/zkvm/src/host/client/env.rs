...
impl<'a> ExecutorEnvBuilder<'a> {
    ...
    ///
    /// Note that [CompositeReceipt][crate::CompositeReceipt] cannot be used as an assumption; it
    ...
    pub fn add_assumption(
        ...
        let assumption_receipt = assumption
            .try_into()
            .map_err(Into::into)
            .context("Failed to convert assumption to AssumptionReceipt")?;
        self.inner
            .assumptions
            .borrow_mut()
            .0
            .push(assumption_receipt);
        Ok(self)
    }
